...
int NetStream::open(void * p)
{
	if (super::open(p) == -1)
	{
		DEF_LOG_ERROR("Failed to call super::open in NetStream::open, param is <%d>, the last error is : <%d>\n", p, ACE_OS::last_error());
		return -1;
	}
...
void NetStream::handleOutputStream(Packet * output_stream)
{
	...
	if (m_output_msg_block.length() == 0)
	{
		...
		if (output_stream->stream_size() != r)
		{
			...
			else
			{
				...
				{
					DEF_LOG_ERROR("Failed to send data in NetStream::handleOutputStream, last error is <%d>\n", ACE_OS::last_error());
				}
			}
		}
		else
...
void NetStream::parseInputPacket()
{
	...
	try
	{
		...
		while(true)
		{
			if (m_input_msg_block.length() <= sizeof(Head))
			{
				break;
			}
	...
	{
		DEF_LOG_ERROR("Raise unknown exception in ClientSession::parseInputPacket, last error is <%d>\n", ACE_OS::last_error());
	}
}
...
int NetStream::rd_stream()
{
	...
	else if (0 == recv_n)
	{
		...
		DEF_LOG_ERROR("Remote connection closed in ClientSession::rd_stream, recv number is 0, last error is <%d>\n", ACE_OS::last_error());
		result = -1;
	}
	else if (recv_n < 0)
	{
		...
		else
		{
			...
			DEF_LOG_ERROR("Remote connection closed in ClientSession::rd_stream, last error is <%d>\n", ACE_OS::last_error());
			result = -1;
			//			}
			//closeHandle();
		}
...
void NetStream::serializeToMsgBlock(Packet * output_stream, int already_read_byte)
{
