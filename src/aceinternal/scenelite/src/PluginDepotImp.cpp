...
int PluginDepotImp::input(Packet * packet)
{
	return -1;
}

int PluginDepotImp::input(const PackInfo & pack_info)
{
//	PDEPOT_LOG_DEBUG("enter PluginDepotImp::input, opcode is <%d>", pack_info.opcode);
	...
	if (result >= 0)
	{
		PDEPOT_LOG_DEBUG("***enter PluginDepotImp::input, player is <%llu>, opcode is <%d>, spend time is <%llu>", pack_info.guid, pack_info.opcode, spend_time);
	}
	else
	{
		PDEPOT_LOG_ERROR("***enter PluginDepotImp::input, player is <%llu>, opcode is <%d>, spend time is <%llu>, input msg error code is <%d>", pack_info.guid, pack_info.opcode, spend_time, result);
		if ((!m_plugin_depot_cfg.scene->isLineScene()) && (m_plugin_depot_cfg.push_client_error_msg))
		{
			MAKE_NEW_PACKET(ps, SMSG_ERROR_DEBUG_MSG, pack_info.guid, "");
			m_plugin_depot_cfg.scene->getMessage()->playerMsg(ps);
		}
	}
	return result;
}

int PluginDepotImp::notify(const PackInfo & pack_info)
{
	...
	if (it != m_notify_msg_map.end())
	{
		...
		for (vector<NotifyMsgInfo>::iterator sub_it = notify_msg.begin(); sub_it != notify_msg.end(); ++sub_it)
		{
			...
			uint64 spend_time = 0;
			time_diff.to_usec(spend_time);
			PDEPOT_LOG_DEBUG("*** enter PluginDepotImp::notify, player is <%llu>, plugin is <%d>, opcode is <%d>, spend time is <%llu>", pack_info.guid, (*sub_it).plugin_object->getType(), pack_info.opcode, spend_time);
		}
	}
	else
	{
...
int PluginDepotImp::request(const PackInfo & pack_info)
{
	int result = -1;
	ACE_Time_Value start_time = ACE_OS::gettimeofday();
	...
	if (result >= 0)
	{
		if (spend_time < 1000)
		{
			PDEPOT_LOG_DEBUG("*** enter PluginDepotImp::request, player is <%llu>, opcode is <%d>, spend time is <%llu>", pack_info.guid, pack_info.opcode, spend_time);
		}
		else
		{
			PDEPOT_LOG_INFO("*** enter PluginDepotImp::request, player is <%llu>, opcode is <%d>, spend time is <%llu>", pack_info.guid, pack_info.opcode, spend_time);
		}
	}
	else
	{
		PDEPOT_LOG_ERROR("*** enter PluginDepotImp::request, player is <%llu>, opcode is <%d>, spend time is <%llu>", pack_info.guid, pack_info.opcode, spend_time);
	}

	return result;
}

int PluginDepotImp::gmcmd(const string & gm_name, const vector<string> & gm_param, uint64 target_guid)
{
...
bool PluginDepotImp::isShutdownSuccess()
{
...
bool PluginDepotImp::isAvailablePlugin(Plugin * plugin)
{
	bool is_valid = false;
	...
	if ((plugin->getSceneType() == PST_ONLY_SCENE) && (!m_plugin_depot_cfg.scene->isLineScene()))
	{
		return true;
	}

	if ((plugin->getSceneType() == PST_ONLY_LINE) && (m_plugin_depot_cfg.scene->isLineScene()))
	{
		return true;
	}
	...
	//if ()
	//{
	//	if ((plugin->getType() == PT_MANAGE_PROXY)
	//		)
	//	{
	//		return false;
	//	}
	...
	//else
	//{
	//	if ((plugin->getType() == PT_MANAGE_PVP)
	//		|| (plugin->getType() == PT_MANAGE_SHORT)
	//		|| (plugin->getType() == PT_MANAGE_MAIL)
	//		|| (plugin->getType() == PT_MANAGE_COPY)
	//		|| (plugin->getType() == PT_MANAGE_FRIEND)
	//		|| (plugin->getType() == PT_MANAGE_GUILD)
	//		|| (plugin->getType() == PT_MANAGE_TOWER)
	//		)
	//	{
	//		return false;
	//	}
